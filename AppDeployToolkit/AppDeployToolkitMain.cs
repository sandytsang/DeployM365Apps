...
namespace PSADT
{
    public class Msi
    {
        ...
        {
            IntPtr hModuleInstance =
                LoadLibraryEx("msimsg.dll",
                IntPtr.Zero,
    ...
    public class Explorer
    {
        ...
        [DllImport("user32.dll", CharSet = CharSet.Auto, SetLastError = false)]
        static extern bool
        SendNotifyMessage(IntPtr hWnd, int Msg, IntPtr wParam, IntPtr lParam);
    ...
    public sealed class FileVerb
    {
        ...
        LoadString(IntPtr h, int id, StringBuilder sb, int maxBuffer);
    ...
    public sealed class IniFile
    {
        ...
        GetIniValue(string section, string key, string filepath)
        {
            string sDefault = "";
    ...
    public class UiAutomation
    {
        ...
        [DllImport("shell32.dll", CharSet = CharSet.Auto, SetLastError = false)]
        static extern int
        SHQueryUserNotificationState(out UserNotificationState pquns);

        [DllImport("user32.dll", CharSet = CharSet.Auto, SetLastError = false)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool
        EnumWindows(EnumWindowsProcD lpEnumFunc, ref IntPtr lParam);
        ...
        [DllImport("user32.dll", CharSet = CharSet.Auto, SetLastError = false)]
        public static extern bool
        EnableMenuItem(IntPtr hMenu, uint uIDEnableItem, uint uEnable);
        ...
        public static List<IntPtr> EnumWindows()
        {
            try
            {
                ...
                EnumWindowsProcD CallBackPtr =
                    new EnumWindowsProcD(EnumWindowsProc);
            ...
            {
                throw new Exception("An error occured during window enumeration: " +
                    ex.Message);
            }
        }
    ...
    public class QueryUser
    {
        ...
        WinStationQueryInformation(
            ...
            int sessionId,
            int information,
            ref WINSTATIONINFORMATIONW pBuffer,
            int bufferLength,
        ...
        public static extern bool
        ProcessIdToSessionId(int processId, ref int pSessionId);
        ...
        private static IList<T> PtrToStructureList<T>(IntPtr ppList, int count)
            where T : struct
        {
            ...
            {
                T item =
                    (T) Marshal.PtrToStructure(new IntPtr(pointer), typeof (T));
                result.Add (item);
                pointer += sizeOf;
            }
            return result;
        ...
        public static WINSTATIONINFORMATIONW
        GetWinStationInformation(IntPtr server, int sessionId)
        {
            int retLen = 0;
            WINSTATIONINFORMATIONW wsInfo = new WINSTATIONINFORMATIONW();
            WinStationQueryInformation(server,
            sessionId,
        ...
        public static TerminalSessionData[] ListSessions(string ServerName)
        {
            ...
            try
            {
                ...
                {
                    sessionsInfo =
                        PtrToStructureList<WTS_SESSION_INFO>(ppSessionInfo,
                        count);
                }
                finally
                {
                ...
                {
                    if (
                        sessionInfo.SessionName != "Services" &&
                        sessionInfo.SessionName != "RDP-Tcp"
                    )
                    {
                        _isUserSession = true;
                    }
                    results
                        .Add(new TerminalSessionData(sessionInfo.SessionId,
        ...
        public static TerminalSessionInfo
        GetSessionInfo(string ServerName, int SessionId)
        {
            IntPtr server = IntPtr.Zero;
            ...
            try
            {
                DirectoryEntry localMachine =
                    new DirectoryEntry("WinNT://" + ServerName + ",Computer");
                ...
                DirectoryEntry admGroup =
                    localMachine.Children.Find(localAdminGroupName, "group");
                ...
                foreach (object groupMember in (IEnumerable) members)
                {
                    ...
                    if (member.Name != String.Empty)
                    {
                        ...
                        {
                            localAdminGroupSidsList
                                .Add((new NTAccount(member.Name))
            ...
            try
            {
                ...
                if (
                    ...
                    SessionId,
                    WTS_INFO_CLASS.ClientBuildNumber,
                    out buffer,
                    out bytesReturned) ==
                    false
                )
                ...
                if (
                    WTSQuerySessionInformation(server,
                    SessionId,
                    WTS_INFO_CLASS.ClientDirectory,
                    out buffer,
                    out bytesReturned) ==
                    false
                )
                ...
                if (
                    WTSQuerySessionInformation(server,
                    SessionId,
                    WTS_INFO_CLASS.ClientProtocolType,
                    out buffer,
                    out bytesReturned) ==
                    false
                )
                ...
                if (strData != String.Empty)
                {
                    ...
                    string _Sid =
                        (new NTAccount(_NTAccount + "\\" + strData))
                            .Translate(typeof (SecurityIdentifier))
                            .Value;
                    data.SID = _Sid;
                ...
                WINSTATIONINFORMATIONW wsInfo =
                    GetWinStationInformation(server, SessionId);
                ...
                DateTime? _disconnectTime =
                    FileTimeToDateTime(wsInfo.DisconnectTime);
        ...
        public static TerminalSessionInfo[]
        GetUserSessionInfo(string ServerName)
        {
            if (ServerName == "localhost" || ServerName == String.Empty)
            {
                ServerName = Environment.MachineName;
            ...
            List<TerminalSessionInfo> userSessionsInfo =
            ...
            foreach (TerminalSessionData session in sessions)
            {
                if (session.IsUserSession == true)
                {
                    ...
                    {
                        if (
                            (firstActiveUserNTAccount == String.Empty) &&
                            (
                            sessionInfo.ConnectState == "Active" ||
                            sessionInfo.ConnectState == "Connected"
                            )
                        )
                        {
